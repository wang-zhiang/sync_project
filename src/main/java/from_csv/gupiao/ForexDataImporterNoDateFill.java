...
public class ForexDataImporterNoDateFill {
    ...
    private static final String CSV_FILE_PATH = "D:\\wzza\\develop\\idea_project\\ceshi\\src\\main\\java\\from_csv\\gupiao\\外汇历史行情5.csv";
    
    public static void main(String[] args) {
        try {
            ForexDataImporterNoDateFill importer = new ForexDataImporterNoDateFill();
    ...
    public void processAndImportData() throws IOException, SQLException {
        ...
        // 插入数据到ClickHouse（不补日期，只导入实际存在的数据）
    ...
    private void insertDataToClickHouse(Map<LocalDate, ForexRecord> dataMap, String code) throws SQLException {
        ...
        try (Connection conn = DriverManager.getConnection(CLICKHOUSE_URL, USERNAME, PASSWORD)) {
            ...
            try (PreparedStatement pstmt = conn.prepareStatement(insertSQL)) {
                ...
                System.out.println("开始插入数据，共 " + dataMap.size() + " 条记录");
                System.out.println("使用code: " + code);
                
                // 按日期排序处理
                List<LocalDate> sortedDates = new ArrayList<>(dataMap.keySet());
                Collections.sort(sortedDates);
                
                for (LocalDate date : sortedDates) {
                    ForexRecord record = dataMap.get(date);
                    
                    // 每1000条记录打印一次进度
                    if (totalRecords % 1000 == 0) {
                        System.out.println("当前处理日期: " + date + ", 已处理记录: " + totalRecords);
                    }
                    ...
                    if (batchCount >= 1000) {
                        pstmt.executeBatch();
                        conn.commit();
                        System.out.println("已插入 " + totalRecords + " 条记录，当前日期: " + date);
                        batchCount = 0;
                    }
                }
