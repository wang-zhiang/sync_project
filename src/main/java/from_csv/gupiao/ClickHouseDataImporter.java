...
//同步到ck的
public class ClickHouseDataImporter {
    ...
    private static final String TABLE_NAME = "sp_data";
    ...
    * */
    ...
    private static final String CSV_FILE_PATH = "D:\\wzza\\develop\\idea_project\\ceshi\\src\\main\\java\\from_csv\\gupiao\\商品价格详情数据.csv";
    
    public static void main(String[] args) {
        try {
            ClickHouseDataImporter importer = new ClickHouseDataImporter();
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (values.length > 0 && !values[0].trim().isEmpty()) {
                    try {
                        ...
                        // 调试：打印特定日期的解析结果
                        if ("2024/12/31".equals(values[0].trim())) {
                            System.out.println("调试 2024/12/31:");
                            System.out.println("解析后列数: " + values.length);
                            System.out.println("头部列数: " + headers.length);
                            for (int i = 0; i < Math.min(values.length, headers.length); i++) {
                                System.out.println("第" + (i+1) + "列 [" + headers[i] + "]: [" + values[i] + "]");
                            }
                            ...
                            if (headers.length > 17) {
                                String lastIndicator = headers[17].trim();
                                String lastValue = values.length > 17 ? values[17].trim() : "无数据";
                                System.out.println("最后字段 [" + lastIndicator + "]: [" + lastValue + "]");
                            }
                        }
