...
public class sql2_ck {

    //public static void main(String[] args) {
    public static String getConf(String cktable1, String sqltable1, String sqlServerUsername1, String sqlServerPassword1) {
        ...
        String sqlServerUrl1 = "jdbc:sqlserver://192.168.4.201:2422;DatabaseName=Taobao_trading";
        String sqlServerUrl2 = "jdbc:sqlserver://smartnew.tpddns.cn:24222;database=Taobao_trading";
        String sqlServerUrl  = get_url.getValidSQLServerURL(sqlServerUrl1,sqlServerUrl2,"CHH","Y1v606",sqltable);
...
//            // 连接ClickHouse数据库
            ...
            while (sqlServerResultSet.next()) {
                ...
                if (dataType1 == "String") {
                    ...
                    columnName ="REPLACE(REPLACE(REPLACE(\\`" + columnName + "\\`, '\\\\\\\\n', ''), '\\\\\\\\t', ''), '\\\\\\\\r', '')";}
                else if (dataType1.equals("UInt8")) {
                    columnName = "CAST(\\`" + columnName + "\\` AS Int)"; // 强制转换为 Int
                }
//                }else if(columnName.equals("price原始")){
...
//                }
            ...
            //
           // conffiled = "sql{ sql=\\" + "\"select " + conffiled +   " from result where substr(adddate,1,7) >= '2023-01' and substr(adddate,1,7) <= '2023-12' \\\"}"   ;
            conffiled = "sql{ sql=\\" + "\"select " + conffiled +   " from result where substr(adddate,1,7) >= '2024-01'  \\\"}"   ;
            ...
            //数据开始同步
            //String query = "query = \\\"" + conffiled + "\\\"";

            String sql1 = "sed \"24s/.*/ " + conffiled + " /\" /opt/module/seatunnel-2.1.2/script_spark/clickhouse/ck/sqlserver-ck-temp.conf > /opt/module/seatunnel-2.1.2/script_spark/clickhouse/ck/sqlserver-ck-temp1.conf ";

            ssh.executeCommand(sql1);
            System.out.println(sql1);

            //清除数据
            //String whereresult = "  where  AddDate >= '2023-01-01' AND AddDate <= '2023-12-31'";
            String whereresult = "  where   AddDate = '2024-01-01' ";
            String stringsql = "alter table " + cktable + " ON CLUSTER test_ck_cluster delete" + whereresult;
            // String stringsql = "truncate table " + ckdatabasetable + " ON CLUSTER test_ck_cluster";

            try {
                cksqlserverUtil.ckEXECUTE(stringsql);
                // 如果没有异常发生，则执行成功
                System.out.println("根据条件删除部分数据" + cktable + "成功");
            } catch (Exception e) {
                System.out.println("覆盖数据失败: " + e.getMessage());
            }


            //生成执行的脚本
             conf = "/opt/module/seatunnel-2.1.2-yarn/bin/start-seatunnel-spark.sh \\\n" +
                    "        --master yarn \\\n" +
                    "        --deploy-mode client \\\n" +
                    "        --config /opt/module/seatunnel-2.1.2/script_spark/clickhouse/ck/sqlserver-ck-temp1.conf \\\n" +
                    "        --variable url=\"" + sqlServerUrl + "\"\\\n" +
                    "        --variable sqlserver_user=\"" + sqlServerUsername + "\"\\\n" +
                    "        --variable sqlserver_password=\"" + sqlServerPassword + "\"\\\n" +
                     "        --variable sqlserver_table.csv=\"" + sqltable + "\"\\\n" +
