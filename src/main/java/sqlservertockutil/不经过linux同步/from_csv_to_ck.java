...
public class from_csv_to_ck {
    ...
    private static final String SQL_SERVER_QUERY_TEMPLATE = "SELECT * FROM %s WHERE ABS(CHECKSUM(Id)) %% 10 = ?"; // 分片查询模板

    // ClickHouse连接详情
    private static final String CLICKHOUSE_URL = "jdbc:clickhouse://hadoop104:8123/test";
    ...
    public static void main(String[] args) {
        ...
        for (Map.Entry<String, String> entry : tableMappings.entrySet()) {
            ...
            String clickHouseTableName = entry.getValue().trim().isEmpty() ? sqlServerTableName : entry.getValue().trim();
    ...
    private static Map<String, String> readCSV(String filePath) {
        ...
        try (CSVReader reader = new CSVReader(new InputStreamReader(new FileInputStream(filePath), StandardCharsets.UTF_8), '\t')) {
            ...
            while ((line = reader.readNext()) != null) {
                ...
                String clickHouseTableName = (line.length > 1 && !line[1].isEmpty()) ? line[1] : sqlServerTableName;
                System.out.println("sqlserver的表名：" + sqlServerTableName);
                System.out.println("ck的表名：" + clickHouseTableName);
