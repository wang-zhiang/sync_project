...
public class mysql_ck {

    private static final int BATCH_SIZE = 1000; // 每批处理的数据量

    public static void main(String[] args) throws Exception {
        String MYSQL_JDBC_URL = "jdbc:mysql://192.168.5.34:3306/elm20210522";
        ...
        String MYSQL_TABLE = "goods";
        String CLICKHOUSE_TABLE = "test.elm_2105";
        ...
        // 获取 MySQL 表的列信息
        String sql = "SELECT COLUMN_NAME, DATA_TYPE, CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'elm20210522' AND TABLE_NAME = '" + MYSQL_TABLE + "'";
        ...
        System.out.println("Creating ClickHouse table if not exists: " + createTableQuery.toString());
        ...
        while (clickhouseColumns.next()) {
            String columnName = clickhouseColumns.getString("name");
            clickhouseColumnSet.add(columnName);
        }
        System.out.println("Retrieved ClickHouse table columns: " + clickhouseColumnSet.toString());
        ...
        // 执行 MySQL 查询
        String selectQuery = "SELECT * FROM " + MYSQL_TABLE;
        System.out.println("Executing MySQL query: " + selectQuery);

        PreparedStatement mysqlSelectStmt = mysqlConn.prepareStatement(selectQuery);
        ResultSet mysqlDataRs = mysqlSelectStmt.executeQuery();

        int batchSize = 0;
        // 将 MySQL 查询结果插入到 ClickHouse 中
    ...
    private static String mapDataType(String mysqlDataType, int columnSize) {
        // 数据类型映射函数，根据实际情况调整
        // 注意：这里的映射可能不是完全准确的，需要根据实际情况进行调整
